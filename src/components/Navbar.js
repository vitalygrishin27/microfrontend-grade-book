        ...
        <div className={"col-md-12 bg-bg-light py-2"}>
            <nav className="navbar navbar-light bg-opacity-100">
                ...
                <div className="mx-3" style={{"textAlign": "right", "display": "inline-block"}}>
                    ...
                    {isLoginIn && token.accessLevel === AccessLevelFilter.ADMIN &&
                        <Link to={rootUrl + "/classes"} className={"btn btn-success text-light"}>{t("Classes")}</Link>}
                    {isLoginIn && token.accessLevel === AccessLevelFilter.ADMIN &&
                        <Link to={rootUrl + "/users"} className={"btn btn-success text-light"}>{t("Users")}</Link>}
                    {isLoginIn && token.accessLevel === AccessLevelFilter.ADMIN &&
                        <Link to={rootUrl + "/subjects"}
                              className={"btn btn-success text-light"}>{t("Subjects")}</Link>}
                    {isLoginIn && token.accessLevel === AccessLevelFilter.TEACHER && <Link to={rootUrl + "/teacher/scheduler"}
                                                                                         className={"btn btn-success text-light"}>{t("Scheduler")}</Link>}
